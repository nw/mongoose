...
Connection = this.Connection = Class({
  ...
  _open: function(options){
    ...
    this.db = new mongo.MongoDB();
    
    this.db.addListener('connection', function(){ 
    ...
    this.db.connect({ hostname: this.uri.hostname, port: this.uri.port, db: this.name });    
  },
...
Collection = this.Collection = Class({
  ...
  setDb: function(db){
    var self = this;
    db.getCollection(this.name, function(err, collection){
...
for (var i in require('mongodb').Collection.prototype){
  (function(name){
